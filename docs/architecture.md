...
| numeric array `LET A(i)=expr` or `LET A(i,j)=expr` with debugger-supported subscript and value expressions      | Evaluate and round each subscript index, evaluate the RHS numeric expression, store the result in the numeric-array-variable map under `(arrayName → subscripts → value)`, and continue to the next higher line; unresolvable subscripts or value expressions result in sequential skip                                                                                                                      |
| string array `LET A$(i)=expr` or `LET A$(i,j)=expr` with debugger-supported subscript and string expressions    | Evaluate and round each subscript index, evaluate the RHS string expression, store the result in the string-array-variable map under `(arrayName → subscripts → value)`, and continue to the next higher line; unresolvable subscripts or value expressions result in sequential skip                                                                                                                        |
...
- array numeric and string `LET` (subscripted targets)
- `CALL KEY`
- `CALL SOUND`
- `RETURN`
- `END`
