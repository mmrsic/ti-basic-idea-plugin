...
- Quick documentation (**Ctrl+Q** / hover) shows TI radix-100 storage details for numeric constants, plus contextual info for character-code arguments, `CALL COLOR`, and `CALL CHAR`/`DATA` hex patterns
...
- **Debug TI-Basic Program** via the usual IntelliJ **Debug** entry points for the current TI-Basic file: opens a dedicated **TI Basic Debug** tool window with a frozen program listing whose source line numbers are emphasized, whose code wraps after 28 characters like on the TI-99/4A, and whose current program-counter line stays vertically centered whenever enough context exists above and below it; shorter listings stay top-aligned and only leave empty space at the bottom when the pane is taller than the whole listing. The debugger also shows a TI screen pane on the right with the initial 24x32 screen state rendered from TI character pixel patterns (including the built-in default patterns and any `CALL CHAR` overrides known to the screen model); right beside that screen, a same-height character-set preview renders all 16×8 characters `32..159` with their current debugger patterns and `CALL COLOR` assignments, resolving transparent foreground/background colors against the active `CALL SCREEN` background. Both visual previews scale to the available pane size, and the screen preview offers a toggle to keep or ignore the original screen aspect ratio. The debugger marks the current program counter line, skips `REM` lines while stepping sequentially, renders screen `PRINT` output into the 28-column text area from column 3 to 30 with TI-style zone handling for `,`, line feeds from `:` separators, plus an implicit trailing line feed unless the last PRINT token is `;`, whole-item wrapping for strings shorter than 28 characters, multi-line output for longer strings, and TI-BASIC numeric formatting including scientific notation and omission of a final trailing blank when that alone would overflow the line, applies `CALL CLEAR` by resetting all 32x24 screen positions to character code 32 while preserving the active screen colors, applies `CALL SCREEN` by updating the screen background with the same rounded TI color resolution as the editor previews, applies `CALL HCHAR` and `CALL VCHAR` directly to the debugger screen and both previews with TI-style row/column wrapping and a 768-cell overwrite cap, applies `CALL CHAR` by updating the debugger-managed pattern overrides for codes `32..159`, shows a full-width footer arguments area only when the current paused line has debugger-supported arguments, renders multiple arguments on separate lines with matching height, currently resolves `CALL SCREEN` to `color-code = XX (NAME)`, `CALL HCHAR`/`CALL VCHAR` to evaluated `row`, `column`, `character-code`, and `repeat` lines, and `CALL CHAR` to evaluated `ascii-code` and effective `pattern-string` lines plus a dedicated 8x8 gray pixel preview with ignored trailing digits called out inline; malformed expressions still appear as `<incorrect expression>` or `<incorrect expression> (string-number-mismatch)`, while invalid `CALL CHAR` values surface as `Bad Value` and stop execution. It shows `IF` conditions as an evaluation trace of all subexpressions in evaluation order with variables substituted by their current values and the final condition result on the last line, shows `FOR` control values as the evaluated `initial-value`, `limit`, and `increment` lines, assigns the evaluated initial value to the control variable when stepping onto the loop, and adds the computed iteration count as a fourth parenthesized line while still defaulting omitted `STEP` clauses to `1`; `NEXT` shows the reused increment plus the adjusted control-variable value and applies that update when stepped, evaluates `CALL COLOR` arguments against the current debug state and updates all 16 TI character sets (`32..159`) with rounded foreground/background colors, plays stepped `CALL SOUND` statements through the same shared sound playback used by the gutter action, evaluates `IF` expressions against the current debug state and branches to `THEN` or `ELSE` targets based on `expression<>0`, with a missing `ELSE` falling through to the next higher line, steps through the remaining line flow with `GOTO`, `GOSUB`, `ON GOTO`, `ON GOSUB`, `RETURN`, `END`, `STOP`, and `CALL KEY`, reusing the same jump handling for direct and computed branches and reporting `Bad Value in <line>` when an `ON` selector rounds outside the target list or `Bad Line Number in <line>` when the selected target line does not exist in the program, reports malformed or unknown statements as `Incorrect Statement`, reports type mismatches in numeric or string contexts as `String-number mismatch`, evaluates string and numeric `LET` assignments, initializes unknown referenced numeric variables to `0`, evaluates string `LET` assignments including concatenation plus `CHR$`, `SEG$`, and `STR$`, truncates every intermediate string result to 255 characters with a debugger warning when needed, shows known string and numeric variables with their TI-Basic internal storage format plus their usual display value, evaluates numeric and string array `LET` assignments (e.g., `LET A(I)=5`, `LET B$(2,3)="HI"`) and resolves array-element references in numeric and string expressions (e.g., `A(I)+3`, `R$(I)`) by evaluating each subscript index and assigned/read value against the current debug state and tracking all known array elements in a dedicated **Array Variables** pane below the scalar-variables split, showing each element as `name(subscripts) = internalDisplay [| usualDisplay]`, and shows a keyboard input pane for `CALL KEY`, including the effective mode and its allowed code ranges; mode `0` reuses the last keyboard mode or defaults to mode `5`, mode `3` accepts codes `1..15` and `32..95`, mode `4` accepts `1..143`, and mode `5` accepts `1..15`, `32..159`, plus `187`; and provides a **Skip** control that automatically advances through `NEXT` loops or `GOSUB`/`ON GOSUB` calls until user input is required or the current control-flow construct is about to finish.
...
- **CALL SCREEN gutter preview** — for every `CALL SCREEN(colorCode)` line a solid 16×16 color square appears in the
  ...
  statically resolvable numeric expression; decimal results are rounded to the nearest TI color index, and `SCREEN(1)`
  is displayed as black because transparent screen backgrounds behave like color `2`; a
...
- **CALL SOUND gutter playback** — for every resolvable `CALL SOUND(dur,pitch1,vol1[,pitch2,vol2...])`, a play icon
  ...
  and `-8` following tone 3; debugger stepping reuses the same playback service for executed `CALL SOUND` lines
...
- **TI Basic Character Definitions tool window** — a dockable bottom panel listing all statically resolvable
  ...
  that can be derived statically from matching `CALL COLOR(set,fg,bg)` statements for the character's set; if a
  `CALL COLOR` uses TI color `1` (Transparent), the currently active `CALL SCREEN` background is used instead, with
  `SCREEN(1)` again treated as black;
